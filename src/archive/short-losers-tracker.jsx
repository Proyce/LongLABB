...
} from "./src/telemetry/entryTelemetry.js";
import {
...
import {
  ...
  flattenRsiTelemetry,
  rsiTelemetryCSVRow,
} from "./src/telemetry/rsiTelemetry.js";
import {
...
} from "./src/telemetry/trendTelemetry.js";
import {
...
} from "./src/telemetry/advancedMarketTelemetry.js";
import { classifyShortBucket } from "./src/shortBucket/shortBucketClassifier.js";
import { evaluateShortGateAudit, buildShortAuditFields } from "./src/shortGate/shortGateAudit.js";
import FiltersTab from "./src/FiltersTab.jsx";
import { SmartTable } from "./src/SmartTable.jsx";
import {
  FEE_CONFIG,
  computeFeeDragMarginPct,
  computeFeeTelemetry,
  computeSimProfileFeeTelemetry,
} from "./src/telemetry/feeTelemetry.js";
import {
  EXIT_PROFILE,
  EXIT_PROFILE_CONFIG,
  PROFIT_LOCK_CONFIG,
...
} from "./src/exitProfiles/dynamicExitProfiles.js";
import {
...
} from "./src/absoluteEntryScore.js";
import {
  ...
  postFee10CSVRow,
} from "./src/scoring/postFee10/index.js";
import {
  BEST_DNA_DEFAULT_FIELDS,
...
} from "./src/audits/bestDnaAudit.js";
import {
  fetchMarketRegimeContext,
  getLastValidSnapshot,
  computeSnapshotAgeMs,
  MARKET_REGIME_CONFIG,
} from "./src/marketRegime/index.js";
import {
  flattenMarketRegimeSnapshot,
  MARKET_REGIME_DEFAULTS,
  MARKET_REGIME_CSV_HEADERS,
  marketRegimeCSVRow,
} from "./src/marketRegime/marketRegime.flatten.js";
import { computeSessionHealth } from "./src/sessionHealth/sessionHealth.governor.js";
import {
  flattenSessionHealth,
  SESSION_HEALTH_DEFAULTS,
  SESSION_HEALTH_CSV_HEADERS,
  sessionHealthCSVRow,
} from "./src/sessionHealth/sessionHealth.flatten.js";
import { computeAdaptiveAes } from "./src/entryPolicy/adaptiveAes.js";
import { evaluateEntryPolicy } from "./src/entryPolicy/entryPolicy.js";
import {
  flattenEntryPolicy,
  flattenAdaptiveAes,
  ENTRY_POLICY_DEFAULTS,
  ADAPTIVE_AES_DEFAULTS,
  ENTRY_POLICY_CSV_HEADERS,
  entryPolicyCSVRow,
} from "./src/entryPolicy/entryPolicy.flatten.js";
import { computeLongAuditDangerLogOnly } from "./src/entryPolicy/longAuditDangerLogOnly.js";
import { computeMarketBreathLogOnly } from "./src/marketRegime/marketBreathLogOnly.js";
import { evaluateSniperShortGateLogOnly } from "./src/entryPolicy/sniperShortGateLogOnly.js";
import { evaluateGainerDiagnosticGate } from "./src/entryPolicy/gainerDiagnosticGate.js";
import { evaluateLoserDiagnosticGate } from "./src/entryPolicy/loserDiagnosticGate.js";
import { computeExecutionRankScoreLogOnly } from "./src/scoring/executionRankScoreLogOnly.js";
import { evaluateEntryPolicyLogOnly } from "./src/entryPolicy/evaluateEntryPolicyLogOnly.js";
import {
  ENTRY_POLICY_V2_CSV_HEADERS,
  flattenEntryPolicyLogOnly,
  entryPolicyLogOnlyCSVRow,
} from "./src/entryPolicy/entryPolicyLogOnly.flatten.js";
import { evaluateLiveExitAudit } from "./src/liveExitAudit/liveExitAudit.evaluate.js";
import {
  LIVE_EXIT_AUDIT_CSV_HEADERS,
  flattenLiveExitAuditCsvRow,
  flattenLiveExitAuditDefaults,
} from "./src/liveExitAudit/liveExitAudit.flatten.js";
import MarketRegimeHeader from "./src/components/MarketRegimeHeader.jsx";
import AesDiscoveryTab from "./src/aesDiscovery/AesDiscoveryTab.jsx";
import ShadowLongAuditTab from "./src/shadowLong/ShadowLongAuditTab.jsx";
import {
  SHADOW_LONG_CONFIG,
  isEarlySlAuditCandidate,
  buildShadowLongAudit,
  updateShadowLongAudit,
} from "./src/shadowLong/shadowLongAudit.js";
import { AES_DISCOVERY_CONFIG, mergeDiscoveryConfig } from "./src/aesDiscovery/aesDiscoveryConfig.js";
import { buildFullShortUniverse, updateTickHistory, computeTickHistoryFields } from "./src/aesDiscovery/aesDiscoveryUniverse.js";
import { selectDeepScanCandidates } from "./src/aesDiscovery/aesDiscoveryPrefilter.js";
import { createQueue } from "./src/aesDiscovery/aesDiscoveryQueue.js";
import { buildDiscoveryTelemetrySnapshot } from "./src/aesDiscovery/aesDiscoveryTelemetry.js";
import { computeDiscoveryAesVariants } from "./src/aesDiscovery/aesDiscoveryScore.js";
import { assignDiscoveryLabels } from "./src/aesDiscovery/aesDiscoveryLabels.js";
import {
  createEpisodeState, maybeShadowEntry,
  updateShadowTradeBroadScan, evaluateShadowExit, updateEpisodeState,
} from "./src/aesDiscovery/aesDiscoveryShadowEngine.js";
import { apiFetch, onRateLimitChange, snapshot as rlSnapshot, RL } from "./src/rateLimiter/index.js";

// ─── CONSTANTS ────────────────────────────────────────────────────────────────
const FAPI       = "https://fapi.binance.com/fapi/v1";
const MIXED_BUCKET_TARGET = 25;
const MAX_SAMPLES = MIXED_BUCKET_TARGET * 2;
const RL_LIMIT   = RL.HARD_CEILING;
...
const SHORT_BUCKET_POSITION_LIMITS = {
  maxOpenShortsTotal: MIXED_BUCKET_TARGET * 2,
  maxOpenShortsPerSymbol: 1,
  parentBucketCaps: {
    TOP_LOSER_SHORTS:  MIXED_BUCKET_TARGET,
    TOP_GAINER_SHORTS: MIXED_BUCKET_TARGET,
  },
};

const SHORT_RISK_CONFIG = {
  basis: "PRICE_PERCENT",
...
const fPct = n => `${n >= 0 ? "+" : ""}${f2(n)}%`;
const fDd  = n => n ? `-${f2(Math.abs(n))}%` : "0.00%";
...
const fVol = v => v >= 1e9 ? `${(v/1e9).toFixed(1)}B` : v >= 1e6 ? `${(v/1e6).toFixed(1)}M` : `${(v/1e3).toFixed(0)}K`;
const fTime = ms => {
  if (ms <= 0) return "DONE";
...
// ─── HELPERS ─────────────────────────────────────────────────────────────────
function getProfitLockUpdate(s, cp, ts) {
  const priceFavor = (s.entryPrice - cp) / s.entryPrice * 100;
  const marginPnl  = priceFavor * s.leverage;
  const rules      = getDynamicProfitLockRules(s);
  ...
  for (const rule of rules) {
    if (priceFavor >= rule.triggerPricePct && rule.lockMarginPct > bestLockMargin) {
      const lockPrice = parseFloat((s.entryPrice * (1 - rule.lockMarginPct / s.leverage / 100)).toFixed(8));
      bestLockMargin = rule.lockMarginPct;
      newLockPrice   = lockPrice;
      newStage       = rule.stage;
      activated      = true;
      events = [...events, {
        ts, symbol: s.symbol, leverage: s.leverage, stage: rule.stage, price: cp,
        priceFavorPct:  parseFloat(priceFavor.toFixed(4)),
        marginPnlPct:   parseFloat(marginPnl.toFixed(4)),
        lockMarginPct:  rule.lockMarginPct, lockPrice, reason: "UPDATED",
      }];
    }
  }
...
function getLossProfile(finalPnlPct, mfe, closeReason) {
  ...
  if (closeReason === "AUTO_END" && (mfe ?? 0) < 0.25) return "SLOW_BLEED";
  if (closeReason === "TIMEOUT"  && (mfe ?? 0) < 0.25) return "SLOW_BLEED";
...
function extractEntryTiming(klines, entryPrice, spreadPct) {
  ...
  const reasons = [];
  let grade;
  if ((microBouncePct ?? 0) > 1.5 && immediateGreenImpulse) {
    grade = "F"; reasons.push("VIOLENT_MICRO_BOUNCE");
  } else if ((preEntryAdverseMovePct ?? 0) > 1.0 && immediateGreenImpulse) {
    grade = "F"; reasons.push("STRONG_ADVERSE_IMPULSE");
  } else if ((microBouncePct ?? 0) > 0.5 || (immediateGreenImpulse && last3TicksDirection !== "DOWN")) {
    grade = "D"; reasons.push(immediateGreenImpulse ? "GREEN_IMPULSE" : "MICRO_BOUNCE");
  } else if (last3TicksDirection === "DOWN" && !immediateGreenImpulse && (preEntryAdverseMovePct ?? 1) < 0.2) {
    grade = "A"; reasons.push("CLEAN_DOWNTREND");
  } else if (last5TicksDirection === "DOWN" && (preEntryAdverseMovePct ?? 1) < 0.5) {
    grade = "B"; reasons.push("MOSTLY_DOWN");
  } else {
...
function getSessionQuality(btc1h, eth1h, avgAtr) {
  if (btc1h > 1.5 && eth1h > 1.5)    return "BOUNCE_TRAP_CANDIDATE";
  if (btc1h < -0.5 && eth1h < -0.5)  return "SHORT_FRIENDLY_CANDIDATE";
  if (avgAtr > 2.5)                   return "HIGH_CHOP_CANDIDATE";
...
function normalizeAutoRunTargetBucket(bucket) {
  ...
  return bucket === "TOP_GAINER_SHORTS" ? "TOP_GAINER_SHORTS" : "TOP_LOSER_SHORTS";
}
...
function autoRunTargetLabel(bucket) {
  ...
  return target === "TOP_GAINER_SHORTS" ? "GAINERS" : "LOSERS";
}
...
function countActiveByParentBucket(samples, bucket) {
  return samples.filter(s => !s.closed && s.shortParentBucket === bucket).length;
}
...
function getAvailableBucketSlots(samples, bucket) {
  ...
  const totalRemaining = SHORT_BUCKET_POSITION_LIMITS.maxOpenShortsTotal - totalActive;
  const bucketCap      = SHORT_BUCKET_POSITION_LIMITS.parentBucketCaps[bucket] ?? 0;
...
function makeBucketRiskFields(lev) {
  ...
  return {
    riskBasis:                    SHORT_RISK_CONFIG.basis,
    slMarginPct:                  pricePctToMarginPct(SHORT_RISK_CONFIG.stopLossPricePct, lev),
    slPricePct:                   SHORT_RISK_CONFIG.stopLossPricePct,
    ...
    trailingDistanceMarginPct:    pricePctToMarginPct(SHORT_RISK_CONFIG.trailingDistancePricePct, lev),
    trailingDistancePricePct:     SHORT_RISK_CONFIG.trailingDistancePricePct,
    profitLockRules,
  };
}
...
function computeExitProfileLabel(bucket, closeReason, finalPnlPct) {
  const side = bucket === "TOP_GAINER_SHORTS" ? "GAINER" : "LOSER";
  if (closeReason === "PROFIT_LOCK") return `${side}_PROFIT_LOCK_EXIT`;
  if (closeReason === "TRAIL")       return `${side}_TRAIL_EXIT`;
  if (closeReason === "SL")          return finalPnlPct < 0 ? `${side}_SL_LOSS` : `${side}_SL_SCRATCH`;
  if (closeReason === "TP")          return `${side}_TP_WIN`;
  if (closeReason === "TIMEOUT")     return finalPnlPct > 0 ? `${side}_TIMEOUT_WIN` : `${side}_TIMEOUT_LOSS`;
...
function computeCloseDiagnostics(s, closeReason, finalPnlPct) {
  ...
  if (closeReason === "AUTO_END" && s.profitLockActive)
    diagnostics.push("AUTO_END_AFTER_LOCK_TRIGGER");
  ...
  return {
    ...
    exitProfileLabel:         computeExitProfileLabel(s.shortParentBucket, closeReason, finalPnlPct),
  };
}

function finalizeClosedSample(mergedSample, closeReason, finalPnlPct, extra = {}) {
  const feeTelemetry = computeFeeTelemetry({
    marginPnlPct: finalPnlPct,
    leverage: mergedSample.leverage,
  });
  const diagnostics = computeCloseDiagnostics(
    { ...mergedSample, ...extra, ...feeTelemetry },
    closeReason,
    finalPnlPct,
  );
  const closedSample = {
    ...
    closed: true,
    closeReason,
    closedAt: extra.closedAt ?? mergedSample.closedAt ?? Date.now(),
    finalPnlPct,
    normPnlPct: feeTelemetry.rawNormPnlPct,
    ...feeTelemetry,
    feeAdjustedFinalPnlPct: feeTelemetry.feeAdjustedMarginPnlPct,
    lossProfile: getLossProfile(finalPnlPct, mergedSample.mfe, closeReason),
    ...diagnostics,
  };
  const outcome = calculatePostFee10OutcomeAssessment(closedSample);
  const bestDnaOutcome = flattenBestDnaOutcomeFields(closedSample);
  return {
    ...closedSample,
    ...bestDnaOutcome,
...
function applyPostFee10EntryScoring(sample, evaluatedAtMs = Date.now()) {
  const snapshot = buildPostFee10EntrySnapshot(sample, { evaluatedAtMs });
  const assessment = calculatePostFee10EntryAssessment(snapshot, DEFAULT_POST_FEE_10_CONFIG);
  return {
    ...sample,
    ...flattenPostFee10EntryAssessment(snapshot, assessment),
  };
}
...
function makeBucketClassificationFields() {
  return {
    ...
    shortParentBucket:               null,
    shortSubBucket:                  "UNCLASSIFIED",
    shortSetupScore:                 null,
    shortSetupReasons:               [],
    shortSetupWarnings:              [],
    ...
    topGainerExhaustionAuditLabel:           null,
...
const getTickers  = ()          => apiGet(`${FAPI}/ticker/24hr`);
const getFunding  = ()          => apiGet(`${FAPI}/premiumIndex`);
...
function runSummaryMetrics(rs, fallbackBtcPrice = null) {
  const closed = rs.filter(s => s.closed);
  const active = rs.filter(s => !s.closed);
  ...
  const totalMae = rs.reduce((a,s) => a + (numberOrNull(s.mae) ?? 0), 0);
  const totalMfe = rs.reduce((a,s) => a + (numberOrNull(s.mfe) ?? 0), 0);
  ...
  return {
    ...
    realizedNetPnl,
    totalNetPnl,
    pnlPerTrade: rs.length ? totalPnl / rs.length : null,
    ...
    totalMae: rs.length ? totalMae : null,
    totalMfe: rs.length ? totalMfe : null,
    avgMae: rs.length ? totalMae / rs.length : null,
    avgMfe: rs.length ? totalMfe / rs.length : null,
    best: bestSample ? samplePnlPct(bestSample) : null,
    worst: worstSample ? samplePnlPct(worstSample) : null,
    bestSample,
    worstSample,
    tpCount: closed.filter(s => ["TP","TRAIL","PROFIT_LOCK"].includes(s.closeReason)).length,
    slCount: closed.filter(s => s.closeReason === "SL").length,
...
function sampleRunSide(s) {
  if (s?.shortParentBucket === "TOP_GAINER_SHORTS" || s?.leaderboardSide === "GAINERS") return "gainers";
  if (s?.shortParentBucket === "TOP_LOSER_SHORTS"  || s?.leaderboardSide === "LOSERS")  return "losers";
  return "other";
}
...
export default function App() {
  ...
  const [rl,          setRl]          = useState({ weight: 0, calls: 0, inflight: 0, waiting: 0, throttling: false, isBanned: false, isBackoff: false, banUntil: 0, backoffUntil: 0 });
  ...
  // ── Shadow LONG Audit state ──────────────────────────────────────────────────
  const [shadowLongAudits, setShadowLongAudits] = useState([]);
  const processedEarlySlIdsRef = useRef(new Set());
  ...
  useEffect(() => {
    (async () => {
      try {
        const s = await window.storage.get("sl_v3:samples");
        if (s?.value) setSamples(JSON.parse(s.value).map(sample => sanitizeSampleRun(sample, run)));
        const w = await window.storage.get("sl_v3:watchlist");
        if (w?.value) setWatchlist(JSON.parse(w.value));
        const r = await window.storage.get("sl_v3:run");
        if (r?.value) setRun(parseInt(r.value) || 1);
        const h = await window.storage.get("sl_v3:holdMs");
  ...
  useEffect(() => {
    if (storageOk) {
      const safeSamples = samples.map(sample => sanitizeSampleRun(sample, run));
      window.storage.set("sl_v3:samples", JSON.stringify(safeSamples)).catch(()=>{});
    }
  }, [samples, run, storageOk]);
  useEffect(() => { if (storageOk) window.storage.set("sl_v3:watchlist", JSON.stringify(watchlist)).catch(()=>{}); }, [watchlist, storageOk]);
  useEffect(() => { if (storageOk) window.storage.set("sl_v3:run",    String(run)).catch(()=>{});    }, [run,    storageOk]);
  useEffect(() => { if (storageOk) window.storage.set("sl_v3:holdMs", String(holdMs)).catch(()=>{}); }, [holdMs, storageOk]);
  ...
  const fetchData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const fullUniverse = buildFullShortUniverse({ tickers: tickData, minQuoteVolume: minVol });
      ...
      setSamples(prev => prev.map(s => {
        ...
        const mae     = Math.max(s.mae || 0, priceUp);
        const mfe     = Math.max(s.mfe || 0, priceDn);
        const ts      = Date.now();

        let trailPeak = s.trailPeak, trailActive = s.trailActive;
        if (trailOn && priceDn >= TP_PCT) {
          trailActive = true;
        ...
        const lockUpdate = getProfitLockUpdate(sampleWithProfile, cp, ts);
        ...
        const runnerCapture = computeRunnerCapturePotential({
          ...sampleWithProfile,
          ...closeBaseRaw,
          evaluatedAtMs: ts,
          timeSinceEntryMs: heldMs,
          currentNormPnlPct: priceDn,
          normalizedMfePct: mfe,
          normalizedMaePct: mae,
          profitLockActive: lockUpdate.profitLockActive,
          currentLockFloorNormPct:
            lockUpdate.profitLockLevelMarginPct != null && s.leverage
              ? lockUpdate.profitLockLevelMarginPct / s.leverage
              : null,
          trailingActive: trailActive,
        });
        const priorRunnerPeak = numberOrNull(s.runnerScorePeak) ?? -Infinity;
        const currentRunnerScore = numberOrNull(runnerCapture.runnerCapturePotentialScore);
        const runnerPeak =
        ...
        if (!trailOn && priceDn >= TP_PCT) {
          beep(880);
          const finalPnlPct = parseFloat((TP_PCT * s.leverage).toFixed(2));
          const merged = { ...sampleWithProfile, ...closeBase, trailPeak, trailActive };
          return finalizeClosedSample(merged, "TP", finalPnlPct, { closedAt: ts, btcExitPrice: sampleBtcPrice });
        }

        // ─── PROFIT LOCK (before TRAIL/SL — floor protects locked profit) ─
        const lockPrice = lockUpdate.profitLockLevelPrice;
        if (lockUpdate.profitLockActive && lockPrice !== null && cp >= lockPrice) {
          beep(440);
          const computedFinalPnlPct = parseFloat(((s.entryPrice - cp) / s.entryPrice * 100 * s.leverage).toFixed(2));
          const lockFloor = lockUpdate.profitLockLevelMarginPct ?? s.profitLockLevelMarginPct ?? null;
          const finalPnlPct = lockFloor != null
            ? parseFloat(Math.max(computedFinalPnlPct, lockFloor).toFixed(2))
            : computedFinalPnlPct;
          const floorExitEnforced = lockFloor != null && finalPnlPct > computedFinalPnlPct;
          const exitEvent = {
            ts, symbol: s.symbol, leverage: s.leverage,
            stage: lockUpdate.profitLockStage, price: cp,
            priceFavorPct: parseFloat(((s.entryPrice - cp) / s.entryPrice * 100).toFixed(4)),
          ...
          const merged = {
            ...
            floorExitEnforced,
            wouldHaveExitedBelowFloor: floorExitEnforced,
            activeLockFloorMarginPct: lockFloor,
            activeLockFloorPrice: lockPrice,
          };
          return finalizeClosedSample(merged, "PROFIT_LOCK", finalPnlPct, { closedAt: ts, btcExitPrice: sampleBtcPrice });
        }

        // ─── TRAIL ────────────────────────────────────────────────────────
        if (trailActive && trailPeak !== null && cp > trailPeak * (1 + TRAIL_PCT / 100)) {
          beep(660);
          const finalPnlPct = parseFloat(((s.entryPrice - cp) / s.entryPrice * 100 * s.leverage).toFixed(2));
          const merged = { ...sampleWithProfile, ...closeBase, trailPeak, trailActive };
          return finalizeClosedSample(merged, "TRAIL", finalPnlPct, { closedAt: ts, btcExitPrice: sampleBtcPrice });
        }

        // ─── SL ───────────────────────────────────────────────────────────
        if (priceUp >= SL_PCT) {
          beep(200);
          const finalPnlPct = parseFloat((-(SL_PCT * s.leverage)).toFixed(2));
          const merged = { ...sampleWithProfile, ...closeBase, trailPeak, trailActive };
          return finalizeClosedSample(merged, "SL", finalPnlPct, { closedAt: ts, btcExitPrice: sampleBtcPrice });
        }
        ...
        if (heldMs >= (s.holdMs ?? HOLD_MS)) {
          const finalPnlPct = parseFloat(((s.entryPrice - cp) / s.entryPrice * 100 * s.leverage).toFixed(2));
          const merged = { ...sampleWithProfile, ...closeBase, trailPeak, trailActive };
          return finalizeClosedSample(merged, "TIMEOUT", finalPnlPct, { closedAt: ts, btcExitPrice: sampleBtcPrice });
        }
  ...
  }, [minVol, trailOn, beep, getLatestBtcPrice]);

  const fetchLosersKlines = useCallback(async losers => {
    const top = losers.slice(0, 15);
    const [klineResults, oiResults, kline5mResults, breadth5mResults, breadth15mResults] = await Promise.all([
      Promise.allSettled(top.map(t => getKlines(t.symbol, "1m", 20))),
      Promise.allSettled(top.map(t => getOI(t.symbol))),
      Promise.allSettled(top.map(t => getKlines(t.symbol, "5m", 2))),
      Promise.allSettled(top.map(t => getKlines(t.symbol, "5m", 25))),
      Promise.allSettled(top.map(t => getKlines(t.symbol, "15m", 25))),
    ]);
    const km  = {};
    const ts = Date.now();
  ...
  useEffect(() => {
    if (loserTickers.length > 0) fetchLosersKlines(loserTickers);
  }, [loserTickers, fetchLosersKlines]);
  useEffect(() => {
    if (gainerTickers.length > 0) fetchGainersKlines(gainerTickers);
  }, [gainerTickers, fetchGainersKlines]);
  useEffect(() => {
  ...
  // ── Shadow LONG Audit persistence ────────────────────────────────────────────
  useEffect(() => {
    if (!storageOk) return;
    window.storage.set("sl_v3:shadowLongAudits", JSON.stringify(shadowLongAudits)).catch(() => {});
  }, [shadowLongAudits, storageOk]);

  useEffect(() => {
    if (!storageOk) return;
    (async () => {
      try {
        const stored = await window.storage.get("sl_v3:shadowLongAudits");
        if (stored?.value) {
          const restored = JSON.parse(stored.value);
          setShadowLongAudits(restored);
          restored.forEach(a => processedEarlySlIdsRef.current.add(a.sourceShortTradeId));
        }
      } catch (_) {}
    })();
  }, [storageOk]);

  // ── AES Discovery deep scan loop ─────────────────────────────────────────────
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      ...
      const { weight: rlWeight, isBanned, isBackoff } = rlSnapshot();
      const rateLimitUsagePct = (rlWeight / RL.HARD_CEILING) * 100;
      ...
      // Process batch
      const batch = queue.getNextBatch({ rateLimitWeightBudget: null, now });
      ...
      await Promise.allSettled(batch.map(async rec => {
        ...
        try {
          ...
          const snapshot = await buildDiscoveryTelemetrySnapshot({
            ...
            computeAdvancedMarketTelemetry, buildShortAuditFields, classifyShortBucket,
  ...
  // ── Shadow LONG: detect newly closed early SL samples ───────────────────────
  useEffect(() => {
    if (!SHADOW_LONG_CONFIG.enabled) return;

    const candidates = samples.filter(sample =>
      sample.closed === true &&
      sample.closeReason === "SL" &&
      sample.closedAt != null &&
      sample.entryTime != null &&
      sample.closedAt - sample.entryTime <= SHADOW_LONG_CONFIG.captureMaxSourceSlDurationMs
    );

    for (const sourceTrade of candidates) {
      const auditId = `shadow-long:${sourceTrade.id}`;
      const alreadyExists = shadowLongAudits.some(a => a.id === auditId);
      if (alreadyExists || processedEarlySlIdsRef.current.has(sourceTrade.id)) continue;

      processedEarlySlIdsRef.current.add(sourceTrade.id);

      const firstTickPrice = priceMapRef.current[sourceTrade.symbol] ?? sourceTrade.currentPrice ?? sourceTrade.entryPrice;
      if (!firstTickPrice) continue;

      const regime = marketRegimeRef.current;
      const marketSnap = {
        btcDirection: regime?.btc?.direction ?? null,
        btcRegime:    regime?.btc?.regime ?? null,
        ethDirection: regime?.eth?.direction ?? null,
        ethRegime:    regime?.eth?.regime ?? null,
      };

      const audit = buildShadowLongAudit(sourceTrade, firstTickPrice, Date.now(), marketSnap, SHADOW_LONG_CONFIG);
      setShadowLongAudits(prev => {
        if (prev.some(a => a.id === audit.id)) return prev;
        return [...prev, { ...audit, status: "ACTIVE" }];
      });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [samples, shadowLongAudits]);

  // ── Shadow LONG: price feed from main polling loop ───────────────────────────
  useEffect(() => {
    if (!SHADOW_LONG_CONFIG.enabled) return;
    const activeAudits = shadowLongAudits.filter(a => a.status === "ACTIVE" || a.status === "PENDING_ENTRY");
    if (!activeAudits.length) return;
    const pm = priceMapRef.current;
    if (!Object.keys(pm).length) return;

    const now = Date.now();
    setShadowLongAudits(prev => prev.map(a => {
      if (a.status !== "ACTIVE" && a.status !== "PENDING_ENTRY") return a;
      const price = pm[a.symbol];
      if (!price) return a;
      return updateShadowLongAudit(a, price, now, SHADOW_LONG_CONFIG);
    }));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [samples]); // piggyback on the same 15s polling cycle

  // ── Add sample ───────────────────────────────────────────────────────────────
  const addSample = useCallback(async (ticker, bucket = "TOP_LOSER_SHORTS") => {
    ...
    const rng24 = h24 - l24;
    const sourceList = bucket === "TOP_GAINER_SHORTS" ? gainerTickers : loserTickers;
    ...
    const entry = {
      ...
      leaderboardSide:     bucket === "TOP_GAINER_SHORTS" ? "GAINERS" : "LOSERS",
      shortParentBucket:   bucket,
      entryRankInBucket:   rank,
      bucketCapAtEntry:    SHORT_BUCKET_POSITION_LIMITS.parentBucketCaps[bucket],
      bucketActiveCountAtEntry: samples.filter(s => !s.closed && s.shortParentBucket === bucket).length,
      ...ENTRY_TELEMETRY_DEFAULTS,
      ...RSI_TELEMETRY_DEFAULTS,
      ...TREND_TELEMETRY_DEFAULTS,
      ...ADVANCED_MARKET_TELEMETRY_DEFAULTS,
    ...
    Promise.allSettled([
      getKlines(ticker.symbol, "1m", 20),
      getDepth(ticker.symbol),
      getOI(ticker.symbol),
      getKlines(ticker.symbol, ENTRY_TELEMETRY_CONFIG.vwapTimeframe, ENTRY_TELEMETRY_CONFIG.vwapLookback),
      ...getEntrySignalKlineRequests(ticker.symbol),
      ...
      const shortThesisFields = buildShortAuditFields({
        shortParentBucket:       bucket,
        ...
        btcShortTailwindScore:   entry.btcShortTailwindScore ?? null,
        atrPct:                  klSig.atrPct          ?? null,
      ...
      console.info("[SHORT_THESIS_AUDIT]", {
        symbol:                   entry.symbol,
        run:                      entry.run,
        entryRank:                entry.entryRank,
        change24h:                entry.change24h,
        shortGateWouldPass:       shortThesisFields.shortGateWouldPass,
        shortGateAuditLabel:      shortThesisFields.shortGateAuditLabel,
        shortGateFailReasons:     shortThesisFields.shortGateFailReasons,
        shortGateScore:           shortThesisFields.shortGateScore,
        shortThesisLaneLabel:     shortThesisFields.shortThesisLaneLabel,
        microMomentumLabel:       shortThesisFields.microMomentumLabel,
        btcShortContextLabel:     shortThesisFields.btcShortContextLabel,
        greenPressureLabel:       shortThesisFields.greenPressureLabel,
        vwapContextLabel:         shortThesisFields.vwapContextLabel,
        entryQualityWarningLabels: shortThesisFields.entryQualityWarningLabels,
      });
      setSamples(prev => prev.map(s => {
        ...
        const postFee10Scored = applyPostFee10EntryScoring(merged, telemetryComputedAt);
        const absResult = computeAbsoluteEntryScore(postFee10Scored);
        return { ...postFee10Scored, ...flattenAbsoluteEntryScore(absResult) };
      }));
    });
    ...
    const id = setInterval(() => {
      ...
      if (ar.phase === "running" && elapsed >= ar.runDurationMs) {
        ...
        setSamples(prev => prev.map(s => {
          ...
          const finalPnlPct = parseFloat(((s.entryPrice - s.currentPrice) / s.entryPrice * 100 * s.leverage).toFixed(2));
          const sampleBtcPrice = btcCurrentPrice ?? s.btcCurrentPrice ?? s.btcPrice ?? null;
          return finalizeClosedSample(
            { ...s, btcCurrentPrice: sampleBtcPrice },
            "AUTO_END",
            finalPnlPct,
            { closedAt: ts, btcExitPrice: sampleBtcPrice },
          );
        }));
      ...
      if (ar.phase === "cooldown" && elapsed >= ar.cooldownMs) {
        ...
        Promise.resolve(startSetRef.current?.({
          targetBucket: ar.targetBucket ?? "TOP_LOSER_SHORTS",
          runOverride: nextRun,
          source: "AUTO_RUN",
          autoRunId: ar.id,
  ...
  const startBucketSet = useCallback(async (bucket, startOptions = {}) => {
    ...
    const sourceList = bucket === "TOP_LOSER_SHORTS"  ? loserTickers
                     : bucket === "TOP_GAINER_SHORTS" ? gainerTickers
                     : [];
    ...
    const leaderboardSide = bucket === "TOP_LOSER_SHORTS" ? "LOSERS" : "GAINERS";
    ...
    try {
      ...
      setSessionMap(prev => ({ ...prev, [setId]: {
        ...
        btcShortTailwindScore: entryMarketContext?.btc?.shortTailwindScore ?? 0,
    ...
    const newEntries = toAdd.map((ticker, idx) => {
      ...
      return {
        ...
        // Entry policy / adaptive AES defaults (will be updated post-telemetry)
        ...ENTRY_POLICY_DEFAULTS,
        ...ADAPTIVE_AES_DEFAULTS,
    ...
    newEntries.forEach(entry => {
      Promise.allSettled([
        getKlines(entry.symbol, "1m", 20),
        getDepth(entry.symbol),
        getOI(entry.symbol),
        getKlines(entry.symbol, ENTRY_TELEMETRY_CONFIG.vwapTimeframe, ENTRY_TELEMETRY_CONFIG.vwapLookback),
        ...getEntrySignalKlineRequests(entry.symbol),
      ]).then(([kl, dep, oi, kl5m, ...entryKlineResults]) => {
        const klSig = kl.status === "fulfilled" ? extractKlineSignals(kl.value) : {};
        let spreadPct = null;
        if (dep.status === "fulfilled" && dep.value.bids?.length && dep.value.asks?.length) {
        ...
        const klinesByInterval   = mapSettledKlinesByInterval(ENTRY_SIGNAL_INTERVALS, entryKlineResults);
        const rsiSnapshot = computeRsiTelemetry({ symbol: entry.symbol, side: "SHORT", klinesByInterval });
        const trendSnapshot = computeTrendTelemetry({
          symbol: entry.symbol, side: "SHORT", klinesByInterval,
          entryPrice: entry.entryPrice, computedAt: telemetryComputedAt,
        });
        const advancedSnapshot = computeAdvancedMarketTelemetry({
          symbol: entry.symbol, side: "SHORT", entryPrice: entry.entryPrice,
          klinesByInterval, oiSnapshotsBySymbol: oiSnapshotRef.current,
          liquidationSnapshotsBySymbol: liquidationSnapshotRef.current,
          oiCurrent: oiVal, computedAt: telemetryComputedAt,
        });
        const etSnapshot = computeEntryTelemetry({
          klines1m, klines5m, entryPrice: entry.entryPrice, side: "SHORT",
        ...
        setSamples(prev => prev.map(s => {
          ...
          const classification = classifyShortBucket(merged);
          const mergedWithClassification = { ...merged, ...classification };
          const auditFields = buildShortAuditFields(mergedWithClassification);
          const fullyMerged = { ...mergedWithClassification, ...auditFields };
          const postFee10Scored = applyPostFee10EntryScoring(fullyMerged, telemetryComputedAt);
          const absResult   = computeAbsoluteEntryScore(postFee10Scored);
          const aesFlat     = flattenAbsoluteEntryScore(absResult);

          // Adaptive AES — market adjustment only, no PnL
          const currentRegime = marketRegimeRef.current;
          const currentSession = sessionHealthRef.current;
          const tradeSide = s.leaderboardSide === "GAINERS" ? "GAINER" : "LOSER";
          const adaptiveAesResult = computeAdaptiveAes({
            baseAes: aesFlat.absoluteEntryScore ?? 50,
            side:    tradeSide,
            marketContext: currentRegime,
            sessionHealth: currentSession,
          });
          const policyResult = evaluateEntryPolicy({
            symbol:       s.symbol,
            side:         tradeSide,
            baseAes:      aesFlat.absoluteEntryScore ?? 50,
            adaptiveAesResult,
            marketContext: currentRegime,
            sessionHealth: currentSession,
            candidateTelemetry: postFee10Scored,
            evaluationTiming:   "POST_CREATE_RETROFIT",
            marketSnapshotId:   s.marketSnapshotId,
            sessionHealthSnapshotId: s.sessionHealthSnapshotId,
          });

          /**
           * LOG-ONLY POLICY V2:
           * These diagnostics are for UI/export/analysis only.
           * They must not affect candidate execution.
           */
          const longAudit   = computeLongAuditDangerLogOnly(fullyMerged);
          const marketBreath = computeMarketBreathLogOnly(currentRegime);

          const candidateWithDiagnostics = {
            ...postFee10Scored,
            ...aesFlat,
            ...longAudit,
            ...marketBreath,
          ...
          const sniper     = evaluateSniperShortGateLogOnly(candidateWithDiagnostics);
          const gainerGate = evaluateGainerDiagnosticGate(candidateWithDiagnostics);
          const loserGate  = evaluateLoserDiagnosticGate(candidateWithDiagnostics);
          ...
          const executionRank = computeExecutionRankScoreLogOnly(rankedCandidate);
          const policyV2      = evaluateEntryPolicyLogOnly({ ...rankedCandidate, ...executionRank });
          ...
          // DO NOT filter by v2Diagnostics.entryPolicyWouldBlock.
          // DO NOT skip by v2Diagnostics.entryPolicyDiagnosticAction.
          // DO NOT reduce capacity by v2Diagnostics.entryPolicyWouldReduceCapacity.
          // DO NOT force sniper mode by v2Diagnostics.entryPolicyWouldSniperOnly.
          // Existing execution flow continues unchanged.

          return {
            ...postFee10Scored,
            ...aesFlat,
            // Preserve base AES explicitly
            absoluteEntryBaseScore: aesFlat.absoluteEntryScore ?? 50,
            ...flattenAdaptiveAes(adaptiveAesResult),
            ...flattenEntryPolicy(policyResult),
            ...v2Diagnostics,
          };
        }));
      });
  ...
  const startBalancedSet = useCallback(async (startOptions = {}) => {
    ...
    const loserResult  = await startBucketSet("TOP_LOSER_SHORTS",  mixedOptions);
    const gainerResult = await startBucketSet("TOP_GAINER_SHORTS", mixedOptions);
  ...
  useEffect(() => {
    startSetRef.current = (options = {}) => {
      ...
      return targetBucket === "TOP_GAINER_SHORTS"
        ? startBucketSet("TOP_GAINER_SHORTS", options)
        : startBucketSet("TOP_LOSER_SHORTS", options);
    };
  }, [startBalancedSet, startBucketSet]);

  // ── Auto-run controls ────────────────────────────────────────────────────────
  const startAutoRun = useCallback(async (targetBucket = "TOP_LOSER_SHORTS") => {
    if (autoRunIsActive(autoRunRef.current)) return;
    ...
    try {
      ...
      result = bucket === "MIXED_25_25"
        ...
        : bucket === "TOP_GAINER_SHORTS"
          ? await startBucketSet("TOP_GAINER_SHORTS", startOptions)
          : await startBucketSet("TOP_LOSER_SHORTS", startOptions);
    } catch (err) {
  ...
  const manualClose = (id, reason) => {
    setSamples(prev => prev.map(s => {
      ...
      return finalizeClosedSample(
        ...
        reason,
        finalPnlPct,
        { closedAt: Date.now(), btcExitPrice: btcPrice },
      );
    }));
    setCloseModal(null);
  };
  ...
  // ── Export helpers ───────────────────────────────────────────────────────────
  const CSV_COLS = [
    "id","run","symbol","leverage","change24h","entryRank","entryPrice","exitPrice",
    "finalPnlPct","normPnlPct","closeReason","holdMs",
    "mae","mfe","funding","cvdLabel","cvdRatio","atrPct","volAccel",
    "spreadPct","bounceFromLow","distFromHigh","utcHour","oiVal","notes","entryTime","closeTime",
    "leverageProfile","entryRankBucket","entryRankWarning",
    "spreadBucket","atrBucket","cvdInterpretation","bounceContext","sessionQuality",
    "warningFlags","lossProfile",
    "profitLockEnabled","profitLockActive","profitLockActivatedAt",
    "profitLockStage","profitLockLevelMarginPct","profitLockLevelPrice",
    "highestProfitPricePct","highestMarginPnlPct",
    "entryTimingGrade","entryTimingReasons",
    "last3TicksDirection","last5TicksDirection","last10TicksDirection",
    "preEntryAdverseMovePct","microBouncePct","priceVsVwapPct",
    "immediateGreenImpulse","immediateRedImpulse",
    "btcEntryPrice","btcCurrentPrice","btcExitPrice","btcRunChangePct","btcRunDirection",
    "isStale","isInvalidMarket","staleReason","uniquePriceCount",
    // ── Bucket / risk fields ──────────────────────────────────────────────
    "leaderboardSide","shortParentBucket","shortSubBucket",
    "entryRankInBucket","bucketCapAtEntry","bucketActiveCountAtEntry",
    "riskBasis","slMarginPct","slPricePct",
    "profitLockTriggerMarginPct","profitLockTriggerPricePct",
    "profitLockFloorMarginPct","profitLockFloorPricePct",
    "trailingDistanceMarginPct","trailingDistancePricePct",
    "shortSetupScore","shortSetupReasons","shortSetupWarnings",
    "topLoserSubBucket","topLoserSetupScore","topLoserWarningFlags",
    "topGainerSubBucket","topGainerExhaustionScore","topGainerContinuationRiskScore",
    "topGainerExhaustionReasons","topGainerContinuationWarnings",
    ...ENTRY_TELEMETRY_CSV_HEADERS,
    ...RSI_TELEMETRY_CSV_HEADERS,
    ...TREND_TELEMETRY_CSV_HEADERS,
    ...ADVANCED_MARKET_TELEMETRY_CSV_HEADERS,
    ...MARKET_CONTEXT_CSV_HEADERS,
    "shortGateWouldPass","shortGateAuditLabel","shortGateFailReasons","shortGateScore",
    "microMomentumLabel","topLoserThesisLaneLabel","btcShortContextLabel","btcShortContextScore",
    "btcShortContextReason","greenPressureLabel","vwapContextLabel","entryQualityWarningLabels",
    "hasMicroMomentum","hasRsiRollover","hasRedConfirmation","hasGreenDanger",
    "isBlindWeaknessShort","isBtcBounceTrapRisk","isCorpseChaseRisk","legacyBtcShortTailwindScore",
    // ── Gainer exhaustion audit ───────────────────────────────────────────
    "shortThesisLaneLabel",
    "topGainerThesisLaneLabel","topGainerExhaustionAuditLabel","topGainerWouldPassExhaustionAudit",
    "topGainerAuditFailReasons","topGainerExhaustionAuditScore",
    ...
    // ── Exit diagnostics (backward-compat) ───────────────────────────────
    "exitProfileLabel","exitDiagnosticLabels","negativeProfitLockExit","profitLockExitBelowFloor",
    "simSafeProfilePnl","simSafeProfileReason","simRunnerProfilePnl","simRunnerProfileReason",
    "simFastLockProfilePnl","simFastLockProfileReason","bestSimExitProfile","actualVsBestSimDelta",
    "feeDragPct","feeAdjustedFinalPnlPct",
    // ── Explicit fee telemetry ────────────────────────────────────────────
    "rawMarginPnlPct","rawNormPnlPct","normPnlPct",
    "feeMode","takerFeeRatePct","roundTripFeeNotionalPct","feeDragMarginPct",
    "feeAdjustedMarginPnlPct","feeAdjustedNormPnlPct",
    "feeAdjustedWin","feeAdjustedLoss","feeStatusLabel","feeDisplayLabel",
    // ── Sim profile fee fields ────────────────────────────────────────────
    "simSafeProfileMarginPnlPct","simSafeProfileNormPnlPct",
    "simSafeProfileFeeAdjustedMarginPnlPct","simSafeProfileFeeAdjustedNormPnlPct",
    "simRunnerProfileMarginPnlPct","simRunnerProfileNormPnlPct",
    "simRunnerProfileFeeAdjustedMarginPnlPct","simRunnerProfileFeeAdjustedNormPnlPct",
    "simFastLockProfileMarginPnlPct","simFastLockProfileNormPnlPct",
    "simFastLockProfileFeeAdjustedMarginPnlPct","simFastLockProfileFeeAdjustedNormPnlPct",
    "bestSimExitProfileByFeeAdjustedPnl","actualVsBestSimFeeAdjustedDelta",
    // ── Dynamic exit profile telemetry ────────────────────────────────────
    "exitProfileSelected","exitProfileInitialBias","exitProfileReason",
    "exitProfileChangedAt","exitProfileHistory",
    "exitProfileDowngradedFrom","exitProfileDowngradeReason",
    "fastProfileActivated","safeProfileActivated","runnerProfileActivated",
    "activeLockFloorMarginPct","activeLockFloorPrice",
    "floorExitEnforced","wouldHaveExitedBelowFloor",
    ...
    // Session Health
    ...SESSION_HEALTH_CSV_HEADERS,
    // Entry Policy + Adaptive AES (V1)
    ...ENTRY_POLICY_CSV_HEADERS,
    // Entry Policy V2 Log-Only Diagnostics
    ...ENTRY_POLICY_V2_CSV_HEADERS,
    // Live Exit Audit V1 Log-Only
    ...LIVE_EXIT_AUDIT_CSV_HEADERS,
  ];
  ...
  const buildCSVBlob = src => {
    const rows = [CSV_COLS, ...src.map(s => {
      const pnl  = parseFloat(samplePnlPct(s).toFixed(2));
      const norm = s.leverage > 0 ? parseFloat((pnl/s.leverage).toFixed(2)) : "";
      const btcMove = sampleBtcMove(s);
      return [
        s.id, normalizeRunValue(s.run, ""), s.symbol, s.leverage, s.change24h, s.entryRank,
        s.entryPrice, s.currentPrice, pnl, norm,
        s.closeReason||"ACTIVE",
        s.closedAt ? s.closedAt-s.entryTime : Date.now()-s.entryTime,
        (s.mae||0).toFixed(2), (s.mfe||0).toFixed(2),
        s.funding??"", s.cvdLabel??"", s.cvdRatio??"",
        s.atrPct??"", s.volAccel??"", s.spreadPct??"",
        s.bounceFromLow??"", s.distFromHigh??"",
        s.utcHour, s.oiVal??"",
        csvEsc(s.notes||""),
        new Date(s.entryTime).toISOString(),
        s.closedAt ? new Date(s.closedAt).toISOString() : "",
        s.leverageProfile??"", s.entryRankBucket??"", s.entryRankWarning??"",
        s.spreadBucket??"", s.atrBucket??"", s.cvdInterpretation??"",
        s.bounceContext??"", s.sessionQuality??"",
        csvEsc(JSON.stringify(s.warningFlags??[])), s.lossProfile??"",
        "true", s.profitLockActive?"true":"false",
        s.profitLockActivatedAt??"", s.profitLockStage??"",
        s.profitLockLevelMarginPct??"", s.profitLockLevelPrice??"",
        s.highestProfitPricePct??"", s.highestMarginPnlPct??"",
        s.entryTiming?.entryTimingGrade??"",
        csvEsc(JSON.stringify(s.entryTiming?.entryTimingReasons??[])),
        s.entryTiming?.last3TicksDirection??"",
        s.entryTiming?.last5TicksDirection??"",
        s.entryTiming?.last10TicksDirection??"",
        s.entryTiming?.preEntryAdverseMovePct??"",
        s.entryTiming?.microBouncePct??"",
        s.entryTiming?.priceVsVwapPct??"",
        s.entryTiming?.immediateGreenImpulse??"",
        s.entryTiming?.immediateRedImpulse??"",
        sampleBtcEntryPrice(s) ?? "",
        s.btcCurrentPrice ?? "",
        s.btcExitPrice ?? "",
        btcMove.known ? parseFloat(btcMove.pct.toFixed(4)) : "",
        btcMove.known ? btcMove.direction : "",
        s.isStale?"1":"0", s.isInvalidMarket?"1":"0",
        s.staleReason??"", s.uniquePriceCount??"",
        // ── Bucket / risk fields ──────────────────────────────────────────
        s.leaderboardSide??"", s.shortParentBucket??"", s.shortSubBucket??"",
        s.entryRankInBucket??"", s.bucketCapAtEntry??"", s.bucketActiveCountAtEntry??"",
        s.riskBasis??"", s.slMarginPct??"", s.slPricePct??"",
        s.profitLockTriggerMarginPct??"", s.profitLockTriggerPricePct??"",
        s.profitLockFloorMarginPct??"", s.profitLockFloorPricePct??"",
        s.trailingDistanceMarginPct??"", s.trailingDistancePricePct??"",
        s.shortSetupScore??"",
        csvEsc(JSON.stringify(s.shortSetupReasons??[])),
        csvEsc(JSON.stringify(s.shortSetupWarnings??[])),
        s.topLoserSubBucket??"", s.topLoserSetupScore??"",
        csvEsc(JSON.stringify(s.topLoserWarningFlags??[])),
        s.topGainerSubBucket??"", s.topGainerExhaustionScore??"", s.topGainerContinuationRiskScore??"",
        csvEsc(JSON.stringify(s.topGainerExhaustionReasons??[])),
        csvEsc(JSON.stringify(s.topGainerContinuationWarnings??[])),
        ...entryTelemetryCSVRow(s),
        ...rsiTelemetryCSVRow(s),
        ...trendTelemetryCSVRow(s),
        ...advancedMarketTelemetryCSVRow(s),
        ...marketContextCSVRow(s),
        s.shortGateWouldPass ?? "",
        s.shortGateAuditLabel ?? "",
        (s.shortGateFailReasons ?? []).join("|"),
        s.shortGateScore ?? "",
        s.microMomentumLabel ?? "",
        s.topLoserThesisLaneLabel ?? "",
        s.btcShortContextLabel ?? "",
        s.btcShortContextScore ?? "",
        csvEsc(s.btcShortContextReason ?? ""),
        s.greenPressureLabel ?? "",
        s.vwapContextLabel ?? "",
        (s.entryQualityWarningLabels ?? []).join("|"),
        s.hasMicroMomentum ?? "",
        s.hasRsiRollover ?? "",
        s.hasRedConfirmation ?? "",
        s.hasGreenDanger ?? "",
        s.isBlindWeaknessShort ?? "",
        s.isBtcBounceTrapRisk ?? "",
        s.isCorpseChaseRisk ?? "",
        s.legacyBtcShortTailwindScore ?? "",
        // ── Gainer exhaustion audit ───────────────────────────────────────
        s.shortThesisLaneLabel ?? "",
        s.topGainerThesisLaneLabel ?? "",
        s.topGainerExhaustionAuditLabel ?? "",
        s.topGainerWouldPassExhaustionAudit ?? "",
        (s.topGainerAuditFailReasons ?? []).join("|"),
        s.topGainerExhaustionAuditScore ?? "",
        s.topGainerExhaustionQualityScore ?? "",
        s.topGainerContinuationDangerScore ?? "",
        s.topGainerNetExhaustionScore ?? "",
        s.topGainerPumpStrengthLabel ?? "",
        s.topGainerEntryBatchLabel ?? "",
        s.topGainerPumpPhaseLabel ?? "",
        s.topGainerMicroExhaustionLabel ?? "",
        s.topGainerContinuationPressureLabel ?? "",
        s.topGainerVwapContextLabel ?? "",
        s.topGainerRsiContextLabel ?? "",
        s.topGainerTrendContextLabel ?? "",
        s.topGainerVolumeFlowContextLabel ?? "",
        s.topGainerStructureContextLabel ?? "",
        s.topGainerBtcContextLabel ?? "",
        (s.topGainerQualityWarningLabels ?? []).join("|"),
        s.hasGainerExhaustionConfirmation ?? "",
        s.hasGainerContinuationDanger ?? "",
        s.hasGainerRedRejection ?? "",
        s.hasGainerRsiRollover ?? "",
        s.hasGainerTrendRollover ?? "",
        s.hasGainerVolumeFade ?? "",
        s.hasGainerFailedBreakout ?? "",
        s.hasGainerLowerHigh ?? "",
        s.hasGainerVwapLoss ?? "",
        // ── Exit diagnostics ─────────────────────────────────────────────
        s.exitProfileLabel ?? "",
        (s.exitDiagnosticLabels ?? []).join("|"),
        s.negativeProfitLockExit ?? "",
        s.profitLockExitBelowFloor ?? "",
        s.simSafeProfilePnl ?? "",
        s.simSafeProfileReason ?? "",
        s.simRunnerProfilePnl ?? "",
        s.simRunnerProfileReason ?? "",
        s.simFastLockProfilePnl ?? "",
        s.simFastLockProfileReason ?? "",
        s.bestSimExitProfile ?? "",
        s.actualVsBestSimDelta ?? "",
        s.feeDragPct ?? "",
        s.feeAdjustedFinalPnlPct ?? "",
        // ── Explicit fee telemetry ──────────────────────────────────────
        s.rawMarginPnlPct ?? "",
        s.rawNormPnlPct ?? "",
        s.normPnlPct ?? norm,
        s.feeMode ?? "",
        s.takerFeeRatePct ?? "",
        s.roundTripFeeNotionalPct ?? "",
        s.feeDragMarginPct ?? "",
        s.feeAdjustedMarginPnlPct ?? "",
        s.feeAdjustedNormPnlPct ?? "",
        s.feeAdjustedWin ?? "",
        s.feeAdjustedLoss ?? "",
        s.feeStatusLabel ?? "",
        csvEsc(s.feeDisplayLabel ?? ""),
        // ── Sim profile fee fields ──────────────────────────────────────
        s.simSafeProfileMarginPnlPct ?? "",
        s.simSafeProfileNormPnlPct ?? "",
        s.simSafeProfileFeeAdjustedMarginPnlPct ?? "",
        s.simSafeProfileFeeAdjustedNormPnlPct ?? "",
        s.simRunnerProfileMarginPnlPct ?? "",
        s.simRunnerProfileNormPnlPct ?? "",
        s.simRunnerProfileFeeAdjustedMarginPnlPct ?? "",
        s.simRunnerProfileFeeAdjustedNormPnlPct ?? "",
        s.simFastLockProfileMarginPnlPct ?? "",
        s.simFastLockProfileNormPnlPct ?? "",
        s.simFastLockProfileFeeAdjustedMarginPnlPct ?? "",
        s.simFastLockProfileFeeAdjustedNormPnlPct ?? "",
        s.bestSimExitProfileByFeeAdjustedPnl ?? "",
        s.actualVsBestSimFeeAdjustedDelta ?? "",
        // ── Dynamic exit profile telemetry ──────────────────────────────
        s.exitProfileSelected ?? "",
        s.exitProfileInitialBias ?? "",
        s.exitProfileReason ?? "",
        s.exitProfileChangedAt ?? "",
        csvEsc(JSON.stringify(s.exitProfileHistory ?? [])),
        s.exitProfileDowngradedFrom ?? "",
        s.exitProfileDowngradeReason ?? "",
        s.fastProfileActivated ? "true" : "false",
        s.safeProfileActivated ? "true" : "false",
        s.runnerProfileActivated ? "true" : "false",
        s.activeLockFloorMarginPct ?? "",
        s.activeLockFloorPrice ?? "",
        s.floorExitEnforced ? "true" : "false",
        s.wouldHaveExitedBelowFloor ? "true" : "false",
        // ── Absolute Entry Score ──────────────────────────────────────────
        ...absoluteEntryScoreCSVRow(s),
        ...bestDnaCSVRow(s),
        ...postFee10CSVRow(s),
        // ── Market Regime V2 ──────────────────────────────────────────────
        ...marketRegimeCSVRow(s),
        // ── Session Health ────────────────────────────────────────────────
        ...sessionHealthCSVRow(s),
        // ── Entry Policy V1 ───────────────────────────────────────────────
        ...entryPolicyCSVRow(s),
        // ── Entry Policy V2 Log-Only Diagnostics ──────────────────────────
        ...entryPolicyLogOnlyCSVRow(flattenEntryPolicyLogOnly(s)),
        // ── Live Exit Audit V1 Log-Only ───────────────────────────────────
        ...(s.liveExitAuditVersion ? flattenLiveExitAuditCsvRow(s) : flattenLiveExitAuditDefaults()),
      ];
    })];
    return new Blob([rows.map(r => r.join(",")).join("\n")], { type:"text/csv" });
  };

  const dlCSV  = (src, name) => Object.assign(document.createElement("a"), { href:URL.createObjectURL(buildCSVBlob(src)), download:name }).click();
  const dlJSON = (src, name) => Object.assign(document.createElement("a"), { href:URL.createObjectURL(new Blob([JSON.stringify(src, null, 2)], { type:"application/json" })), download:name }).click();

  const exportCSV  = () => dlCSV(rankedSamples,  `shortlab_all_${Date.now()}.csv`);
  const exportJSON = () => dlJSON(rankedSamples, `shortlab_all_${Date.now()}.json`);

  // ── Shadow LONG exports ───────────────────────────────────────────────────────
  const SHADOW_LONG_CSV_COLS = [
    "id","version","status","symbol",
    "sourceShortTradeId","sourceShortRun","sourceShortSetId",
    "sourceShortParentBucket","sourceShortSubBucket",
    "sourceShortEntryTime","sourceShortClosedAt","sourceShortDurationMs","sourceShortDurationLabel",
    "sourceShortEntryPrice","sourceShortExitPrice","sourceShortLeverage",
    "sourceShortGrossMarginPnlPct","sourceShortFeeNetMarginPnlPct","sourceShortFeeNetNormPnlPct",
    "shadowLongHypothesisEligible","shadowLongHypothesisFailReasons",
    "atrPct","atrBucket","shadowLongAtrClass","aes","absoluteEntryScore","entryRank","change24h",
    "spreadPct","cvdLabel","priceVsVwapPct","last3TicksDirection",
    "immediateRedImpulse","immediateGreenImpulse","redImpulseDetected","greenImpulseDetected",
    "btcDirection","btcRegime","ethDirection","ethRegime","marketLivePnlState","marketNetPnlState",
    "shadowLongSignalTime","shadowLongEntryTime","shadowLongEntryReferencePrice",
    "shadowLongEntryPrice","shadowLongTriggerDelayMs","shadowLongLeverage",
    "pnlAt15sNormPct","pnlAt30sNormPct","pnlAt60sNormPct","pnlAt120sNormPct",
    "pnlAt180sNormPct","pnlAt300sNormPct","pnlAt600sNormPct",
    "feeNetPnlAt15sNormPct","feeNetPnlAt30sNormPct","feeNetPnlAt60sNormPct",
    "feeNetPnlAt120sNormPct","feeNetPnlAt180sNormPct","feeNetPnlAt300sNormPct","feeNetPnlAt600sNormPct",
    "grossMfeNormPct","grossMaeNormPct","mfeAtrMultiple","maeAtrMultiple",
    "mirrorCloseReason","mirrorGrossNormPnlPct","mirrorFeeNetNormPnlPct","mirrorFeeNetMarginPnlPct",
    "atrProfileCloseReason","atrProfileGrossNormPnlPct","atrProfileFeeNetNormPnlPct","atrProfileFeeNetMarginPnlPct",
    "combinedFeeNetNormPnlPct","combinedAdditiveMarginPnlPct","combinedCompoundedMarginPnlPct",
    "shortLossRecoveryRatio","fullyRecoveredShortLoss","profitableAfterFullRescue",
    "outcomeLabel","diagnosticLabels","priceSource","samplingPrecision","dataWarnings",
  ];

  const buildShadowLongCSVBlob = () => {
    const rows = [SHADOW_LONG_CSV_COLS, ...shadowLongAudits.map(a =>
      SHADOW_LONG_CSV_COLS.map(col => {
        const v = a[col];
  ...
  const exportShadowLongCSV  = () => Object.assign(document.createElement("a"), {
    href: URL.createObjectURL(buildShadowLongCSVBlob()),
    download: `shortlab_shadow_long_audit_${Date.now()}.csv`,
  }).click();

  const exportShadowLongJSON = () => dlJSON(shadowLongAudits.map(a => {
    const { priceHistory: _, ...rest } = a;
    return rest;
  }), `shortlab_shadow_long_audit_${Date.now()}.json`);
  ...
  const rankedSamples = useMemo(() => {
    ...
    return assignRunBestNormRanks(assignAllPostFee10WinnerRanks(outcomeEnriched));
  }, [samples]);
  ...
  const wins    = closedSamples.filter(s => s.finalPnlPct > 0);
  const losses  = closedSamples.filter(s => s.finalPnlPct <= 0);
  const winRate = closedSamples.length ? wins.length/closedSamples.length*100 : 0;
  const avgPnlLev  = closedSamples.length ? closedSamples.reduce((s,x)=>s+x.finalPnlPct,0)/closedSamples.length : 0;
  const avgPnlNorm = closedSamples.length ? closedSamples.reduce((s,x)=>s+x.finalPnlPct/x.leverage,0)/closedSamples.length : 0;

  const cleanSamples = useMemo(() =>
    closedSamples.filter(s => !s.isStale && !s.isInvalidMarket),
  [closedSamples]);
  ...
  const icResults = useMemo(() => {
    ...
    const base = cleanIcOnly ? cleanSamples : closedSamples;
    ...
    return IC_SIGNALS.map(sig => {
      const pairs = closedEnriched
        .filter(s => s[sig.key] != null)
        .map(s => [parseFloat(s[sig.key]), s.finalPnlPct]);
  ...
  }, [closedSamples, cleanSamples, cleanIcOnly]);

  const equityCurve = useMemo(() => {
    let cum = 0;
    return [...closedSamples].sort((a,b)=>a.closedAt-b.closedAt).map((s,i) => {
      cum += s.finalPnlPct;
      return { n:i+1, equity:parseFloat(cum.toFixed(2)), pnl:parseFloat(s.finalPnlPct.toFixed(2)), sym:s.symbol.replace("USDT","") };
    });
  }, [closedSamples]);

  const scatterData = useMemo(() =>
    closedSamples.map(s => ({ x:s.change24h, y:s.finalPnlPct, sym:s.symbol.replace("USDT","") })),
  [closedSamples]);

  const rankBuckets = useMemo(() =>
    [{label:"Top 1–5",min:1,max:5},{label:"Rank 6–15",min:6,max:15},{label:"Rank 16–30",min:16,max:30}]
    .map(b => {
      const g   = closedSamples.filter(s => s.entryRank>=b.min && s.entryRank<=b.max);
  ...
  const profitLockStats = useMemo(() => {
    const activated    = closedSamples.filter(s => s.profitLockActive);
    const exits        = closedSamples.filter(s => s.closeReason === "PROFIT_LOCK");
    ...
    const missedOpportunities = closedSamples.filter(s =>
      s.finalPnlPct < 0 && (s.mfe ?? 0) >= 1.0 && s.closeReason !== "PROFIT_LOCK"
    );
    return { activated: activated.length, exits: exits.length,
      savedFromSL: savedFromSL.length, totalSaved: parseFloat(totalSaved.toFixed(2)),
  ...
  const postFee10BySymbol = useMemo(() => {
    const out = {};
    [...rankedSamples]
      .filter(s => s.postFee10EntryScore != null)
  ...
  const postFee10Analytics = useMemo(
    () => buildPostFee10AnalyticsReport(closedSamples),
    [closedSamples],
  );
  const isAutoRunActive = autoRunIsActive(autoRun);
  const availableStartSlots = Math.max(0, MAX_SAMPLES - allActive.length);
  const loserActiveCount  = countActiveByParentBucket(allActive, "TOP_LOSER_SHORTS");
  const gainerActiveCount = countActiveByParentBucket(allActive, "TOP_GAINER_SHORTS");
  const loserBucketSlots  = getAvailableBucketSlots(samples, "TOP_LOSER_SHORTS");
  const gainerBucketSlots = getAvailableBucketSlots(samples, "TOP_GAINER_SHORTS");
...
const canStartLoserSet  = Boolean(loserTickers.length)  && !isAutoRunActive && loserBucketSlots  > 0;
  ...
  const portUnrealized = allActive.reduce((sum, s) => {
    const priceDn = (s.entryPrice - s.currentPrice) / s.entryPrice * 100;
    return sum + priceDn * s.leverage;
  }, 0);
    ...
    <div style={S.root}>
      ...
      <div style={S.header}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:5,marginLeft:4}}>
            <Pip color={storageOk?"#4488ff":"#7888c0"} label={storageOk?"SAVED":"..."} />
        ...
        <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap"}}>
          ...
          {shadowLongAudits.length > 0 && (() => {
            const active    = shadowLongAudits.filter(a => a.status === "ACTIVE" || a.status === "PENDING_ENTRY").length;
            const completed = shadowLongAudits.filter(a => a.status === "COMPLETED");
            const flipSum   = completed.reduce((s, a) => s + (a.combinedCompoundedMarginPnlPct ?? 0), 0);
            const chipColor = active > 0 && !completed.length ? "#5599ff" : flipSum > 0 ? "#00ff88" : flipSum < 0 ? "#ff4455" : "#8899cc";
            return (
              <button onClick={() => setTab("long-audit")} style={{
                background:"none", border:`1px solid ${chipColor}55`, borderRadius:3,
                color:chipColor, fontFamily:"'Space Mono',monospace", fontSize:8,
                fontWeight:700, letterSpacing:0.8, padding:"3px 9px", cursor:"pointer", flexShrink:0,
              }}>
                L-AUDIT {active}A · {completed.length}C
                {completed.length > 0 && ` · FLIP ${flipSum >= 0 ? "+" : ""}${flipSum.toFixed(1)}%`}
              </button>
            );
          })()}
      ...
      <div style={S.tabs}>
        {[
          ...
          {id:"long-audit",    label:`LONG AUDIT ${shadowLongAudits.filter(a=>a.status==="ACTIVE"||a.status==="PENDING_ENTRY").length}A · ${shadowLongAudits.filter(a=>a.status==="COMPLETED").length}C`},
        ].map(t => (
      ...
      {/* Content */}
      <div style={S.content}>
        {error && <div style={{color:"#ff4455",fontFamily:"Space Mono",fontSize:10,padding:"6px 0",marginBottom:8}}>{error}</div>}
        ...
        {tab === "losers" && (
          <div>
            {(() => {
              ...
              const c = sess.quality==="SHORT_FRIENDLY_CANDIDATE"?"#00ff88":sess.quality==="BOUNCE_TRAP_CANDIDATE"?"#ff4455":sess.quality==="HIGH_CHOP_CANDIDATE"?"#ffa500":"#a8a8c8";
              return (
                  ...
                  {watchlist.map(w => (
                    <div key={w.symbol} style={S.watchChip}>
                      ...
                      <button
                        ...
                        onClick={()=>{const t=loserTickers.find(x=>x.symbol===w.symbol);if(t)addSample(t);}}>→SHORT</button>
              ...
              <>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  ...
                  <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                    {(!autoRun || autoRun.phase === "done") ? (
                      <button
                        ...
                        onClick={() => startAutoRun("TOP_LOSER_SHORTS")}
                        style={{
                  ...
                  <button
                    ...
                    onClick={() => startBucketSet("TOP_LOSER_SHORTS")}
                    style={{
                  ...
                  <button
                    ...
                    onClick={() => startBucketSet("TOP_GAINER_SHORTS")}
                    style={{
                ...
                <LiveTickerTable
                  ...
                  ctx={{
                    ...
                    bucket: 'TOP_LOSER_SHORTS',
                    btnColor: '#00ff88', btnLabel: '↓ SHORT',
                    actBtn: S.actBtn,
          ...
          <div>
              ...
              <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                {(!autoRun || autoRun.phase === "done") ? (
                  <button
                    ...
                    onClick={() => startAutoRun("TOP_GAINER_SHORTS")}
                    style={{
                      background: "transparent",
                      border:     "1px solid #aa88ff",
                      color:      "#aa88ff",
            ...
            <LiveTickerTable
              ...
              ctx={{
                ...
                bucket: 'TOP_GAINER_SHORTS',
                btnColor: '#00cc66', btnLabel: '↑ SHORT',
                actBtn: S.actBtn,
          ...
          <div>
            ...
            {filtered.length === 0 && (
              <div style={{color:"#9090c8",fontFamily:"Space Mono",fontSize:11,textAlign:"center",padding:"50px 0"}}>
                No samples yet — go to LOSERS and click ↓ SHORT
              </div>
            )}
            {filtered.length > 0 && (
              ...
              <>
                ...
                {[...closedSamples].reverse().map(s => (
                  <ClosedCard key={s.id} s={s}
                    ...
                    setNoteText={setNoteText}
                    saveNote={()=>saveNote(s.id)}
                    linkedAudit={shadowLongAudits.find(a => a.sourceShortTradeId === s.id) ?? null}
                ...
                <div style={S.modal} onClick={e=>e.stopPropagation()}>
                  <div style={{color:"#fff",fontWeight:700,fontSize:14,marginBottom:10}}>
                    ...
                    <span style={{color:"#c0c4d8",fontSize:11,fontWeight:400,marginLeft:8}}>
                      {fPct((closeModal.entryPrice-closeModal.currentPrice)/closeModal.entryPrice*100*closeModal.leverage)} live
                    </span>
                  </div>
        ...
        {tab === "runs" && (() => {
          ...
          const runPillMeta = Object.fromEntries(
            allRuns.map(r => {
              const rrc = rankedSamples.filter(s => sameRunValue(s.run, r) && s.closed);
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {allRuns.length === 0 ? (
                  ...
                  const runsTableRows = allRuns.map(r => {
                    ...
                    return { r, rs, allRunSamples, dateStr: runDateLabel(allRunSamples), m };
                  });
                  ...
                  const runsOverviewCols = [
                    ...
                    { key:"date",       label:"DATE",      width:96,  firstClickDir:"asc",  sortValue:row=>row.allRunSamples[0]?.entryTime??0,
                      render:row=><span style={{color:"#8899cc",fontSize:9}}>{row.dateStr}</span> },
                    ...
                    { key:"total",      label:"TRADES",    width:62,  firstClickDir:"desc", sortValue:row=>row.m.total,
                      render:row=><span style={{color:"#4488ff"}}>{row.m.total}</span> },
                    ...
                    { key:"feeFlips",   label:"FEE FLIPS",  width:72,  firstClickDir:"desc", sortValue:row=>row.m.feeFlipCount??0,
                      render:row=><span style={{color:row.m.feeFlipCount>0?"#ff8833":"#556655"}}>{row.m.feeFlipCount>0?row.m.feeFlipCount:"-"}</span> },
                    { key:"livePnl",    label:"LIVE PnL",   width:74,  firstClickDir:"desc", sortValue:row=>row.m.livePnl??0,
                      render:row=><span style={{color:row.m.livePnl>=0?"#00ff88":"#ff4455"}}>{row.m.active?fPct(row.m.livePnl):"-"}</span> },
                    ...
                    { key:"avgNormPnl", label:"NORM NET",   width:74,  firstClickDir:"desc", sortValue:row=>row.m.avgNormPnl??-Infinity,
                      render:row=><span style={{color:row.m.avgNormPnl!=null&&row.m.avgNormPnl>=0?"#55ff88":"#ff8866"}}>{row.m.avgNormPnl!=null?fPct(row.m.avgNormPnl):"-"}</span> },
                    { key:"avgMae",     label:"AVG MAE",   width:70,  firstClickDir:"asc",  sortValue:row=>row.m.avgMae??-Infinity,
                      render:row=><span style={{color:"#ff8855"}}>{row.m.avgMae!=null?fPct(row.m.avgMae):"-"}</span> },
                    { key:"avgMfe",     label:"AVG MFE",   width:70,  firstClickDir:"desc", sortValue:row=>row.m.avgMfe??-Infinity,
                      render:row=><span style={{color:"#55ff88"}}>{row.m.avgMfe!=null?fPct(row.m.avgMfe):"-"}</span> },
                    { key:"totalMae",   label:"CUM DD",    width:70,  firstClickDir:"asc",  sortValue:row=>row.m.totalMae??-Infinity,
                      render:row=><span style={{color:"#ff6644",fontWeight:700}}>{row.m.totalMae!=null?fDd(row.m.totalMae):"-"}</span> },
                    { key:"totalMfe",   label:"CUM MFE",   width:70,  firstClickDir:"desc", sortValue:row=>row.m.totalMfe??-Infinity,
                      render:row=><span style={{color:"#00ff88",fontWeight:700}}>{row.m.totalMfe!=null?fPct(row.m.totalMfe):"-"}</span> },
                    ...
                    { key:"export",     label:"EXPORT",    width:90,
                      render:row=>(
                        <div style={{display:"flex",gap:4}} onClick={e=>e.stopPropagation()}>
                          <Btn onClick={()=>dlCSV(row.rs,`shortlab_run${row.r}_${runSideFilter}_${Date.now()}.csv`)} c="#00ff88" disabled={!row.rs.length}>CSV</Btn>
                          <Btn onClick={()=>dlJSON(row.rs,`shortlab_run${row.r}_${runSideFilter}_${Date.now()}.json`)} c="#4488ff" disabled={!row.rs.length}>JSON</Btn>
                        </div>
                      )},
                  ];
                  ...
                  return (
                    <>
                      ...
                      <div style={{display:"flex",justifyContent:"flex-end",alignItems:"center",gap:8}}>
                        <span style={{fontSize:7,color:"#334466",letterSpacing:2.5,fontWeight:700,textTransform:"uppercase"}}>EXPORT ALL</span>
                        <Btn onClick={()=>dlCSV(aggregateSource,`shortlab_all_${runSideFilter}_${Date.now()}.csv`)} c="#00ff88" disabled={!aggregateSource.length}>↓ CSV</Btn>
                        <Btn onClick={()=>dlJSON(aggregateSource,`shortlab_all_${runSideFilter}_${Date.now()}.json`)} c="#4488ff" disabled={!aggregateSource.length}>↓ JSON</Btn>
                      </div>
                    </>
                  );
                })()}
            ...
            {runTab !== "all" && (() => {
              ...
              const rs  = filterSamplesByRunSide(allRunSamples, runSideFilter);
              const rc  = rs.filter(s => s.closed);
              const ra  = rs.filter(s => !s.closed);
              ...
              const livePnl = ra.reduce((a,s)=>{
                const priceDn = (s.entryPrice - s.currentPrice) / s.entryPrice * 100;
                return a + priceDn * s.leverage;
              },0);
              const netPnl = realizedPnl + livePnl;
              const netPnlPct = rs.length ? netPnl/rs.length : 0;
              const avg = rc.length ? realizedPnl/rc.length : 0;
              const avgN= rc.length ? rc.reduce((a,s)=>a+s.finalPnlPct/s.leverage,0)/rc.length : 0;
              const avgMae = rs.length ? rs.reduce((a,s)=>a+(s.mae||0),0)/rs.length : null;
              const avgMfe = rs.length ? rs.reduce((a,s)=>a+(s.mfe||0),0)/rs.length : null;
              const totalMae = rs.length ? rs.reduce((a,s)=>a+(s.mae||0),0) : null;
              const totalMfe = rs.length ? rs.reduce((a,s)=>a+(s.mfe||0),0) : null;
              ...
              const tpCount = rc.filter(s=>["TP","TRAIL"].includes(s.closeReason)).length;
              const slCount = rc.filter(s=>s.closeReason==="SL").length;
              const aeCount = rc.filter(s=>s.closeReason==="AUTO_END").length;
              const toCount = rc.filter(s=>s.closeReason==="TIMEOUT").length;
              const tag     = `shortlab_run${runTab}_${runSideFilter}_${Date.now()}`;
              ...
              return (
                <div>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(145px,1fr))",gap:9}}>
                    <StatCard label="TOTAL"  value={rs.length} sub={`${ra.length} active`}                                         c="#4488ff" />
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(145px,1fr))",gap:9}}>
                    ...
                    <StatCard label="AUTO END"   value={aeCount} sub="force-closed"                                c="#aa88ff" />
        ...
        {tab === "stats" && (
          <div>
            ...
            <div style={S.grid}>
              ...
              <StatCard label="TP / TRAIL" value={closedSamples.filter(s=>["TP","TRAIL"].includes(s.closeReason)).length}
                sub={`${closedSamples.length?f2(closedSamples.filter(s=>["TP","TRAIL"].includes(s.closeReason)).length/closedSamples.length*100):0}%`} c="#00ff88" />
              <StatCard label="SL HITS"    value={closedSamples.filter(s=>s.closeReason==="SL").length}
                sub={`${closedSamples.length?f2(closedSamples.filter(s=>s.closeReason==="SL").length/closedSamples.length*100):0}%`} c="#ff4455" />
              <StatCard label="TIMEOUTS"   value={closedSamples.filter(s=>s.closeReason==="TIMEOUT").length}   sub="3h expiry"                                    c="#ffa500" />
            ...
            {closedSamples.length > 0 && (<>
                  ...
                  <Sect title="SPREAD BUCKET">
                    <div style={{display:"flex",gap:7,flexWrap:"wrap",marginTop:8}}>
                      {rows.map(({b,arr}) => {
                        const wr  = arr.filter(s=>s.finalPnlPct>0).length/arr.length*100;
                        const avg = arr.reduce((s,x)=>s+x.finalPnlPct,0)/arr.length;
                        const slRate = arr.filter(s=>s.closeReason==="SL").length/arr.length*100;
                        return (
              ...
              <Sect title="PARENT BUCKET COMPARISON">
                {(() => {
                  const buckets = ["TOP_LOSER_SHORTS", "TOP_GAINER_SHORTS"];
                  const med = arr => {
                  ...
                  return (
                    <div style={{overflowX:"auto",marginTop:8}}>
                      <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"Space Mono",fontSize:9,color:"#9898b8"}}>
                        ...
                        <tbody>
                          {buckets.map(b => {
                            const bc = b === "TOP_LOSER_SHORTS" ? "#ff4455" : "#00cc66";
                            const actv = rankedSamples.filter(s => !s.closed && s.shortParentBucket === b);
                            const clsd = rankedSamples.filter(s => s.closed  && s.shortParentBucket === b);
                            const n    = clsd.length;
                            ...
                            const slr  = n ? clsd.filter(x=>x.closeReason==="SL").length/n*100 : null;
                            const lkr  = n ? clsd.filter(x=>x.closeReason==="PROFIT_LOCK").length/n*100 : null;
                            const trr  = n ? clsd.filter(x=>x.closeReason==="TRAIL").length/n*100 : null;
                            const tir  = n ? clsd.filter(x=>x.closeReason==="TIMEOUT").length/n*100 : null;
              ...
              <Sect title="SUB-BUCKET STATS (analysis only — need 50+ closed trades)">
                {(() => {
                  const subMap = {};
                  rankedSamples.filter(s => s.closed && s.shortSubBucket && s.shortSubBucket !== "UNCLASSIFIED").forEach(s => {
                    if (!subMap[s.shortSubBucket]) subMap[s.shortSubBucket] = { parent: s.shortParentBucket ?? "", trades: [] };
                    subMap[s.shortSubBucket].trades.push(s);
                  });
                      ...
                      <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"Space Mono",fontSize:9,color:"#9898b8"}}>
                        ...
                        <tbody>
                          {entries.map(([sub, {parent, trades}]) => {
                            ...
                            const slr = n ? trades.filter(x=>x.closeReason==="SL").length/n*100 : null;
        ...
        {/* â•â•â• FILTERS â•â•â• */}
        {tab === "filters" && (
          <FiltersTab samples={samples} shadowLongAudits={shadowLongAudits} />
        )}

        {/* LONG AUDIT */}
        {tab === "long-audit" && (
          <ShadowLongAuditTab shadowLongAudits={shadowLongAudits} />
        )}
...
function ActiveCard({ s, now, trailOn, onClose, onRemove, noteEditing, noteText, onEditNote, setNoteText, saveNote }) {
  ...
  const priceDn     = (s.entryPrice - s.currentPrice) / s.entryPrice * 100;
  const livePnl     = priceDn * s.leverage;
      ...
      <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:6,flexWrap:"wrap"}}>
            ...
            <span style={{display:"inline-flex",flexDirection:"column",gap:1}}>
              ...
              <span style={{color:liveNetPnl>=0?"#00cc66":"#cc3344",fontFamily:"Space Mono",fontSize:9,opacity:0.85}}>
                net {liveNetPnl>=0?"+":""}{liveNetPnl.toFixed(2)}%{liveFeeFlipped?" ⚠":""}
              </span>
            </span>
            ...
            {s.profitLockActive && (
              <span style={{color:"#55ff88",fontSize:8,background:"#001408",padding:"2px 6px",borderRadius:2,letterSpacing:1}}>
                LOCK {s.profitLockStage} ≥{fPct(s.profitLockLevelMarginPct??0)}
              </span>
            )}
            ...
            {s.sessionQuality && s.sessionQuality !== "MIXED_SESSION" && (
              <span style={{fontSize:7,color:s.sessionQuality==="SHORT_FRIENDLY_CANDIDATE"?"#00ff88":s.sessionQuality==="BOUNCE_TRAP_CANDIDATE"?"#ff4455":"#ffa500",letterSpacing:1}}>{s.sessionQuality.slice(0,8)}</span>
            )}
          </div>

          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:5}}>
            <SF label="ENTRY"  v={fPrice(s.entryPrice)}   c="#a0aaba" />
            <SF label="NOW"    v={fPrice(s.currentPrice)} c={priceDn>0?"#00ff88":"#ff4455"} />
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            ...
            {s.cvdLabel        && <SF label="CVD"    v={s.cvdLabel}          c={s.cvdLabel==="BEAR"?"#ff4455":s.cvdLabel==="BULL"?"#00ff88":"#888"} />}
            {s.atrPct!=null    && <SF label="ATR%"   v={f3(s.atrPct)}        c={s.atrPct>2?"#ffa500":s.atrPct>1?"#cc8844":"#99b4c8"} />}
          ...
          {s.marketContext && (
            <div style={{
              ...
              <span>SCORE:{s.btcShortTailwindScore ?? 0}</span>
...
function ClosedCard({ s, onRemove, noteEditing, noteText, onEditNote, setNoteText, saveNote, linkedAudit, onOpenAudit }) {
  ...
  const rc     = ["TP","TRAIL"].includes(s.closeReason) ? "#00ff88"
               : s.closeReason === "SL"       ? "#ff4455"
               : s.closeReason === "AUTO_END" ? "#aa88ff"
               : "#ffa500";
        ...
        <span style={{display:"inline-flex",flexDirection:"column",gap:1}}>
          ...
          {netPnl != null && (
            <span style={{color:netPnl>=0?"#00cc66":"#cc3344",fontFamily:"Space Mono",fontSize:9,opacity:0.85}}>
              net {netPnl>=0?"+":""}{netPnl.toFixed(2)}%{feeFlipped?" ⚠ FEE_FLIPPED":""}
            </span>
          )}
        </span>
...
// ─── RATE LIMIT BADGE ────────────────────────────────────────────────────────
const RL_STATUSES = [
  { threshold: 90, label: "CRIT",   color: "#ff2233", bg: "#140003", glow: "#ff223355" },
  { threshold: 75, label: "HIGH",   color: "#ff6600", bg: "#130600", glow: "#ff660044" },
  { threshold: 50, label: "WARN",   color: "#ffd000", bg: "#0e0b00", glow: "#ffd00033" },
  { threshold:  0, label: "OK",     color: "#00ff88", bg: "#020e07", glow: "#00ff8822" },
];
const RL_BANNED  = { label: "BANNED",  color: "#ff00ff", bg: "#140014", glow: "#ff00ff66" };
const RL_BACKOFF = { label: "BACKOFF", color: "#ff8800", bg: "#130a00", glow: "#ff880055" };

// Soft ceiling tick index (where the throttle boundary sits on the bar)
const RL_SOFT_TICK = Math.floor((RL.SOFT_CEILING / RL.HARD_CEILING) * 10); // tick 6

const RateLimitBadge = ({ rl }) => {
  const override   = rl.isBanned ? RL_BANNED : rl.isBackoff ? RL_BACKOFF : null;
  const wPct       = Math.min(100, (rl.weight / RL_LIMIT) * 100);
  const cPct       = Math.min(100, (rl.calls  / RL_LIMIT) * 100);
  const pct        = override ? 100 : Math.max(wPct, cPct);
  const status     = override ?? RL_STATUSES.find(s => pct >= s.threshold);
  const { label, color: c, bg, glow } = status;
  const isCrit     = pct > 90 || !!override;
  const isActive   = rl.inflight > 0 || rl.waiting > 0;
  const isIdle     = pct === 0 && !isActive && !rl.isBanned && !rl.isBackoff;
  const ticks      = 10;

  const display = rl.isBanned
    ? `BAN ${new Date(rl.banUntil).toLocaleTimeString()}`
    : rl.isBackoff
    ? `WAIT ${Math.ceil((rl.backoffUntil - Date.now()) / 1000)}s`
    : isIdle
    ? "· · ·"
    : rl.weight > 0
    ? `${rl.weight.toLocaleString()} / ${RL_LIMIT.toLocaleString()}`
    : `${rl.calls}/min`;

  const displayColor = isIdle ? "#2a3a55" : c;

  return (
    <div className={isIdle ? "rl-idle" : ""} style={{
      display: "flex", alignItems: "center", gap: 7,
      background: bg, border: `1.5px solid ${isIdle ? "#1a2035" : c + "88"}`,
      borderRadius: 5, padding: "0 10px",
      fontFamily: "Space Mono", flexShrink: 0,
      width: 400, height: 28, boxSizing: "border-box",
      overflow: "hidden",
      boxShadow: isIdle ? "none" : `0 0 ${isActive ? 12 : 5}px ${glow}`,
      transition: "box-shadow 0.25s, border-color 0.3s",
    }}>
      {/* Status label — minWidth only, no overflow clip; lets Syne render at natural width */}
      <span style={{
        color: isIdle ? "#2a3a55" : c,
        ...
        minWidth: 44, flexShrink: 0, whiteSpace: "nowrap",
      }}>{label}</span>

      {/* Segmented fuel bar with idle scan overlay */}
      <div style={{ position: "relative", display: "flex", gap: 2, alignItems: "center", flexShrink: 0 }}>
        {Array.from({ length: ticks }).map((_, i) => {
          const segThreshold = (i / ticks) * 100;
          const filled       = pct > segThreshold;
          const segPct       = (i + 1) / ticks * 100;
          const segColor     = segPct > 90 ? "#ff2233" : segPct > 75 ? "#ff6600" : segPct > 50 ? "#ffd000" : "#00ff88";
          const softMark     = i === RL_SOFT_TICK;
          return (
            <div key={i} style={{
              width: 8, height: 13, borderRadius: 2,
              background: filled ? segColor : "#0d0d1e",
              boxShadow: filled ? `0 0 4px ${segColor}99` : "none",
              transition: "background 0.1s, box-shadow 0.1s",
              borderLeft: softMark ? "2px solid #ffd00044" : "none",
            }}/>
          );
        })}
        {isIdle && (
      ...
      {/* Primary metric — 88px fits "1,600 / 1,600" and "1002/min" */}
      <span style={{
        color: displayColor, fontWeight: 700, fontSize: 10,
        letterSpacing: 0.3,
        textShadow: isIdle ? "none" : `0 0 4px ${c}55`,
        width: 88, flexShrink: 0, overflow: "hidden",
        whiteSpace: "nowrap", display: "inline-block",
        transition: "color 0.3s",
      }}>{display}</span>

      {/* rpm/throttle — conditional, only rendered when weight or throttle active */}
      {!isIdle && !override && (rl.weight > 0 || rl.throttling) && (
        <div style={{ display: "flex", gap: 4, alignItems: "center", flexShrink: 0 }}>
          {rl.weight > 0 && rl.calls > 0 && (
            <span style={{ color: "#5566aa", fontSize: 8 }}>{rl.calls}/min</span>
          )}
          {rl.throttling && (
            <span style={{ color: "#ffd000", fontSize: 8, fontWeight: 700 }}>⚡THR</span>
          )}
        </div>
      )}

      {/* Live activity — always in layout, visibility-toggled to hold space */}
      <div style={{
        ...
        {rl.waiting > 0 && (
          <span style={{ color: "#aa88ff", fontSize: 9, fontWeight: 700 }}>⋯{rl.waiting}</span>
        )}
      </div>
    </div>
  );
...
const Pip  = ({color,pulse,label}) => (
  <div style={{display:"flex",alignItems:"center",gap:6,fontSize:8,letterSpacing:2,fontWeight:700,color}}>
    <span style={{position:"relative",display:"inline-flex",alignItems:"center",justifyContent:"center",width:7,height:7,flexShrink:0}}>
      ...
      {pulse && <span style={{position:"absolute",inset:0,borderRadius:"50%",border:`1px solid ${color}`,animation:"ringPulse 2s ease-out infinite",opacity:0}}/>}
    </span>
    {label}
  </div>
);
const Chip = ({children}) => <span style={{fontSize:9,color:"#9ab4d4",background:"#0e0e1c",padding:"2px 9px",borderRadius:4,fontFamily:"Space Mono",border:"1px solid #1c1c34",letterSpacing:0.5}}>{children}</span>;
const Btn  = ({children,onClick,c,disabled}) => (
  <button onClick={onClick} disabled={disabled} style={{background:"transparent",border:`1px solid ${c||"#1c1c30"}`,color:c||"#9aabcc",padding:"3px 10px",borderRadius:4,cursor:"pointer",fontFamily:"'Syne',sans-serif",fontSize:9,letterSpacing:0.5,opacity:disabled?0.25:1,transition:"all 0.18s",boxShadow:c&&!disabled?`0 0 10px ${c}16`:"none"}}>
    {children}
  </button>
);
const TBtn = ({children,on,c,onClick,disabled}) => (
  <button onClick={onClick} disabled={disabled} className="btn-lev" style={{background:on?`linear-gradient(135deg,${c}ee,${c}99)`:"transparent",color:on?"#05050c":c,border:`1px solid ${on?c:"#1c1c30"}`,padding:"3px 10px",borderRadius:4,cursor:disabled?"not-allowed":"pointer",fontFamily:"Space Mono,monospace",fontSize:10,fontWeight:700,transition:"all 0.18s",opacity:disabled?0.32:1,boxShadow:on?`0 2px 14px ${c}44`:"none"}}>
    {children}
  </button>
);
const Lbl  = ({children}) => <span style={{fontSize:8,color:"#7898b8",letterSpacing:2.5,fontWeight:700,textTransform:"uppercase"}}>{children}</span>;
const StatCard = ({label,value,sub,c}) => (
  <div style={{...S.stat,borderTop:`2px solid ${c}44`}} className="slide-in stat-hover">
    ...
    <div style={{fontSize:7,color:"#7898b8",letterSpacing:2.5,marginBottom:7,textTransform:"uppercase",position:"relative"}}>{label}</div>
    ...
    <div style={{fontSize:9,color:"#6888a8",marginTop:5,position:"relative"}}>{sub}</div>
  </div>
);
...
const SF = ({label,v,c}) => (
  <span style={{display:"inline-flex",flexDirection:"column",gap:1}}>
    <span style={{fontSize:7,color:"#8ab4cc",letterSpacing:1.5,textTransform:"uppercase"}}>{label}</span>
    <span style={{color:c||"#d4dcea",fontFamily:"Space Mono",fontSize:10,fontWeight:700}}>{v}</span>
  </span>
);
// ─── LIVE TICKER TABLE ───────────────────────────────────────────────────────
...
function LiveTickerTable({ cols, tickers, ctx }) {
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <div style={{ fontFamily: "Space Mono", fontSize: 7, color: "#3a4060", letterSpacing: 0.5, marginBottom: 3, textAlign: "right" }}>
        ⠿ DRAG COLS · DRAG EDGE TO RESIZE
      </div>
      {/* Header */}
      <div style={{ display: "flex", background: "linear-gradient(90deg,#080810,#09090f)", borderBottom: "1px solid #131326", borderRadius: "4px 4px 0 0" }}>
        {orderedCols.map(col => {
...
// ─── AES V3 PREVIEW ──────────────────────────────────────────────────────────
// Always returns a non-null result. Missing kline booleans stay null (unknown),
// never false — missing red is not confirmed absence of red.
function tickerPreviewScore(kl, ticker, rankIndex, side) {
  const snapshot = buildAbsoluteEntryPreviewSnapshot({ kl: kl ?? {}, ticker: ticker ?? {}, rankIndex, side });
  return computeAbsoluteEntryScore(snapshot);
}

// Compatibility aliases per spec
const tickerAesPreviewScore = (...args) => tickerPreviewScore(...args);

function tickerBestDnaPreviewAssessment(kl, ticker, rankIndex, side) {
  if (!kl || !kl.cvdLabel) return null;
  return evaluateBestDnaAudit({
    shortParentBucket: side === "GAINERS" ? "TOP_GAINER_SHORTS" : "TOP_LOSER_SHORTS",
    leaderboardSide: side,
    cvdLabel: kl.cvdLabel,
    atrPct: kl.atrPct ?? null,
    volAccel: kl.volAccel ?? null,
    candleColorAtEntry: kl.candleColorAtEntry ?? "UNKNOWN",
    immediateRedImpulse: kl.immediateRedImpulse ?? false,
    immediateGreenImpulse: kl.immediateGreenImpulse ?? false,
    redImpulseDetected: kl.redImpulseDetected ?? false,
    greenImpulseDetected: kl.greenImpulseDetected ?? false,
    last3TicksDirection: kl.last3TicksDirection ?? "MIXED",
    failedBreakout1m: kl.failedBreakout1m ?? false,
    failedBreakout3m: kl.failedBreakout3m ?? false,
...
function postFee10TickerDisplay(ctx, ticker, rankIndex, side) {
  const exact =
    ...
    null;
  if (exact?.postFee10PotentialScoreV2 != null) {
    return {
      source: "ENTRY",
      title: "POST-FEE 10+",
      score: exact.postFee10PotentialScoreV2,
      tier: exact.postFee10PotentialTier,
      labels: exact.postFee10PotentialLabels ?? [],
      positives: exact.postFee10PositiveGenes ?? [],
      penalties: exact.postFee10PenaltyGenes ?? [],
    };
  }
  ...
  return {
    source: "PREVIEW",
    title: "POST-FEE 10+",
    score: preview.postFee10PotentialScoreV2,
    tier: preview.postFee10PotentialTier,
    labels: preview.postFee10PotentialLabels ?? [],
    positives: preview.postFee10PositiveGenes ?? [],
    penalties: preview.postFee10PenaltyGenes ?? [],
  };
}
...
function bestDnaTickerDisplay(ctx, ticker, rankIndex, side) {
  const exact =
    ...
    ctx.postFee10BySymbol?.[ticker.symbol] ??
    null;
  if (exact?.bestDnaScore != null) {
    return {
      source: "ENTRY",
      title: "BEST DNA",
      score: exact.bestDnaScore,
      tier: exact.bestDnaTier,
      labels: exact.bestDnaLabels ?? [],
      positives: exact.bestDnaPositiveGenes ?? [],
      penalties: exact.bestDnaPenaltyGenes ?? [],
    };
  }
  ...
  return {
    source: "PREVIEW",
    title: "BEST DNA",
    score: preview.bestDnaScore,
    tier: preview.bestDnaTier,
    labels: preview.bestDnaLabels ?? [],
    positives: preview.bestDnaPositiveGenes ?? [],
    penalties: preview.bestDnaPenaltyGenes ?? [],
  };
}
...
function runnerTickerDisplay(ctx, ticker, rankIndex, side) {
  const exact =
    ...
    ctx.postFee10BySymbol?.[ticker.symbol] ??
    null;
  const score = exact?.candidateRunnerScoreAtScan ?? exact?.candidateRunnerScore ?? null;
  if (score == null) return null;
  return {
    source: exact.closed ? "ENTRY" : "SCAN",
    title: "RUNNER ENTRY",
    score,
    tier: exact.candidateRunnerTier ?? exact.candidateRunnerTierAtScan ?? null,
    labels: exact.candidateRunnerLabels ?? [],
    positives: exact.candidateRunnerPositiveGenes ?? [],
    penalties: exact.candidateRunnerPenaltyGenes ?? [],
...
const DistFromHighBar = ({pct}) => {
  ...
  return (
    <div style={{display:"inline-flex",alignItems:"center",gap:5}}>
      ...
      <span style={{color:"#a8c0d4",fontSize:8,fontFamily:"Space Mono"}}>{pct!=null?pct.toFixed(1):"—"}%</span>
    </div>
  );
};
const BounceBar = ({pct}) => (
  <div style={{display:"inline-flex",alignItems:"center",gap:5}}>
    ...
    <span style={{color:"#a8c0d4",fontSize:8,fontFamily:"Space Mono"}}>{pct.toFixed(0)}%</span>
  </div>
);

// ─── STYLES ──────────────────────────────────────────────────────────────────
const S = {
  root:         { background:"#06060c", minHeight:"100vh", color:"#dde4f4", fontFamily:"'Syne',sans-serif", fontSize:13 },
  ...
  logo:         { width:32, height:32, background:"linear-gradient(135deg,#ff4455 0%,#cc2244 60%,#ff2255 100%)", borderRadius:6, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontSize:14, fontWeight:800, flexShrink:0 },
  title:        { fontSize:15, fontWeight:800, color:"#f0f4ff", letterSpacing:2.5 },
  subtt:        { fontSize:8, color:"#8899bb", letterSpacing:1.5, marginTop:2 },
  ...
  tabs:         { display:"flex", borderBottom:"1px solid #111120", background:"linear-gradient(180deg,#0c0c1a 0%,#08080e 100%)", padding:"0 20px", overflowX:"auto", gap:2 },
  tab:          { background:"transparent", border:"none", padding:"11px 16px", cursor:"pointer", fontFamily:"'Syne',sans-serif", fontSize:9, letterSpacing:1.5, fontWeight:700, whiteSpace:"nowrap" },
  content:      { padding:"14px 20px", maxHeight:"calc(100vh - 140px)", overflowY:"auto" },
  ...
  thdr:         { display:"flex", padding:"5px 8px", marginBottom:2, color:"#6a9ccc", fontSize:8, letterSpacing:2, fontWeight:700, borderBottom:"1px solid #111120", background:"#090910" },
  trow:         { display:"flex", padding:"8px 8px", borderBottom:"1px solid #0d0d1a", alignItems:"center", transition:"background 0.15s" },
  actBtn:       { padding:"3px 9px", borderRadius:3, cursor:"pointer", fontFamily:"Space Mono,monospace", fontSize:9, fontWeight:700, background:"transparent", transition:"all 0.15s", border:"1px solid" },
  ...
  secLbl:       { fontSize:8, color:"#6a9ccc", letterSpacing:3, fontWeight:700, marginBottom:9, textTransform:"uppercase", display:"flex", alignItems:"center", gap:8 },
...
const CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pulse-ring::after {
    ...
    border:1px solid currentColor; animation:ringPulse 2s ease-out infinite; opacity:0;
  }
  ...
  @keyframes neonGlow {
    0%,100% { box-shadow:0 0 6px #00ff8820; }
    50%      { box-shadow:0 0 18px #00ff8840, 0 0 32px #00ff8810; }
  }
  @keyframes gradientBorder {
    0%   { border-color:#ff445580; }
    50%  { border-color:#ff44aa80; }
    100% { border-color:#ff445580; }
  }
  @keyframes iconSpin { to { transform:rotate(360deg); } }

  @keyframes rlIdlePulse {
    0%,100% { opacity: 0.45; }
    50%     { opacity: 0.72; }
  }
  @keyframes rlBarScan {
  ...
  .logo-glow {
    box-shadow: 0 0 0 1px #ff445530, 0 0 20px #ff445528, 0 3px 10px rgba(0,0,0,0.7);
  }
  .header-net-glow {
    box-shadow: inset 0 0 20px rgba(0,255,136,0.04), 0 0 0 1px #0f1822;
  }
  ...
  .run-badge {
    ...
    padding:3px 10px; font-family:'Space Mono',monospace; font-size:9px; font-weight:700;
  ...
  input:focus { outline:1px solid #4488ff; }
